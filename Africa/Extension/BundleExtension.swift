...
extension Bundle {
    func decodeSliderData(_ file: String) -> [SliderModel] {
    ...
    func decodeAnimalData(_ file: String) -> [AnimalModel] {
        //Locate the json file
        guard let url = self.url(forResource: file, withExtension: nil) else {
            fatalError("Failed to locate \(file) in bundle")
        }
        ...
        //create proeprty for decoded data
        guard let decodedData = try? decoder.decode([AnimalModel].self
                                                    , from: data) else {
            fatalError("Failed to decode data from \(file)")
        }
